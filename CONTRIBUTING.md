...
| `nox -s free-threading-pytest-run-parallel` | Similar to `nox -s free-threading`, except that it runs tests with the `pytest-run-parallel` plugin to run tests in parallel across multiple threads as a means of stress testing. |
