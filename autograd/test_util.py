...
def check_vjp(f, x):
    ...
    x_vs, y_vs = vspace(x), vspace(y)
    rng = RandomState(GRAD_CHECK_SEED)
...
def check_jvp(f, x):
    ...
    x_v = vspace(x).randn(RandomState(GRAD_CHECK_SEED))
...
def check_equivalent(x, y):
    ...
    v = x_vs.randn(RandomState(GRAD_CHECK_SEED))
...
def check_grads(f, x, modes=["fwd", "rev"], order=2):
    ...
    if "fwd" in modes:
        ...
        if order > 1:
            ...
            v = vspace(x).randn(RandomState(GRAD_CHECK_SEED))
    ...
    if "rev" in modes:
        ...
        if order > 1:
            ...
            v = vspace(f(x)).randn(RandomState(GRAD_CHECK_SEED))
            check_grads(grad_f, (0, 1), modes, order=order - 1)(x, v)
