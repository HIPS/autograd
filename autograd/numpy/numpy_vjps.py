...
defvjp(anp.add,         lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: g))
defvjp(anp.multiply,    lambda ans, x, y : unbroadcast_f(ans, x, lambda g: y * g),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: x * g))
defvjp(anp.subtract,    lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: -g))
defvjp(anp.divide,      lambda ans, x, y : unbroadcast_f(ans, x, lambda g:   g / y),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: - g * x / y**2))
defvjp(anp.maximum,     lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g * balanced_eq(x, ans, y)),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: g * balanced_eq(y, ans, x)))
defvjp(anp.minimum,     lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g * balanced_eq(x, ans, y)),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: g * balanced_eq(y, ans, x)))
defvjp(anp.fmax,        lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g * balanced_eq(x, ans, y)),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: g * balanced_eq(y, ans, x)))
defvjp(anp.fmin,        lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g * balanced_eq(x, ans, y)),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: g * balanced_eq(y, ans, x)))
defvjp(anp.logaddexp,   lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g * anp.exp(x-ans)),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: g * anp.exp(y-ans)))
defvjp(anp.logaddexp2,  lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g * 2**(x-ans)),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: g * 2**(y-ans)))
defvjp(anp.true_divide, lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g / y),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: - g * x / y**2))
defvjp(anp.mod,         lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: -g * anp.floor(x/y)))
defvjp(anp.remainder,   lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g),
                        lambda ans, x, y : unbroadcast_f(ans, y, lambda g: -g * anp.floor(x/y)))
defvjp(anp.power,
    lambda ans, x, y : unbroadcast_f(ans, x, lambda g: g * y * x ** anp.where(y, y - 1, 1.)),
    lambda ans, x, y : unbroadcast_f(ans, y, lambda g: g * anp.log(replace_zero(x, 1.)) * x ** y))
...
def grad_einsum(argnum, ans, operands_, kwargs):
    ...
    def vjp(g):
        ...
        if isinstance(operands[0], string_types):  # using "ijk" convention.
            ...
            return unbroadcast(anp.einsum(new_subscripts, *new_operands), ans_meta, result_meta)
...
def unbroadcast(x, broadcasted_meta, target_meta, broadcast_idx=0):
    _, broadcasted_ndim, _, broadcasted_iscomplex = broadcasted_meta
    target_shape, target_ndim, dtype, target_iscomplex = target_meta
    leading_dims = anp.ndim(x) - broadcasted_ndim
    broadcast_idx = broadcast_idx + leading_dims
    while anp.ndim(x) > leading_dims + target_ndim:
        x = anp.sum(x, axis=broadcast_idx)
    for axis, size in enumerate(target_shape):
        if size == 1:
            x = anp.sum(x, axis=leading_dims + axis, keepdims=True)
    if anp.iscomplexobj(x) and not target_iscomplex:
        x = anp.real(x)
    return x

def unbroadcast_f(broadcasted, target, f):
    broadcasted_meta = anp.metadata(broadcasted)
    target_meta = anp.metadata(target)
    return lambda g: unbroadcast(f(g), broadcasted_meta, target_meta)
