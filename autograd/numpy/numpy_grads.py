...
anp.divide.defgrad(lambda ans, x, y : unbroadcast(ans, x, lambda g : anp.conj(g) / y))
anp.divide.defgrad(lambda ans, x, y : unbroadcast(ans, y, lambda g : - anp.conj(g) * x / y**2), argnum=1)
anp.power.defgrad(lambda ans, x, y : unbroadcast(ans, x, lambda g : g * y * x ** (y - 1)))
anp.power.defgrad(lambda ans, x, y : unbroadcast(ans, y, lambda g : anp.conj(g) * anp.log(x) * x ** y), argnum=1)
...
anp.reciprocal.defgrad(lambda ans, x : lambda g : - anp.conj(g) / x**2)
...
anp.sin.defgrad(   lambda ans, x : lambda g : anp.conj(g) * anp.cos(x))
...
anp.square.defgrad( lambda ans, x : lambda g : anp.conj(g) * 2 * x)
