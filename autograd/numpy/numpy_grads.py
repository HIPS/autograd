...
anp.multiply.defgrad(lambda ans, x, y : unbroadcast(ans, x, lambda g : anp.conj(y) * g))
anp.multiply.defgrad(lambda ans, x, y : unbroadcast(ans, y, lambda g : anp.conj(x) * g), argnum=1)
...
anp.divide.defgrad(lambda ans, x, y : unbroadcast(ans, x, lambda g :   g / anp.conj(y)))
anp.divide.defgrad(lambda ans, x, y : unbroadcast(ans, y, lambda g : - g * anp.conj(x / y**2)), argnum=1)
anp.power.defgrad(lambda ans, x, y : unbroadcast(ans, x, lambda g : g * anp.conj(y * x ** (y - 1))))
anp.power.defgrad(lambda ans, x, y : unbroadcast(ans, y, lambda g : g * anp.conj(anp.log(x) * x ** y)), argnum=1)
...
anp.exp.defgrad(   lambda ans, x : lambda g : anp.conj(ans) * g)
anp.exp2.defgrad(  lambda ans, x : lambda g : anp.conj(ans) * anp.log(2) * g)
anp.expm1.defgrad( lambda ans, x : lambda g : anp.conj(ans + 1) * g)
anp.log.defgrad(   lambda ans, x : lambda g : g / anp.conj(x))
anp.log2.defgrad(  lambda ans, x : lambda g : g / anp.conj(x) / anp.log(2))
anp.log10.defgrad( lambda ans, x : lambda g : g / anp.conj(x) / anp.log(10))
anp.log1p.defgrad( lambda ans, x : lambda g : g / anp.conj((x + 1)))
anp.sin.defgrad(   lambda ans, x : lambda g : g * anp.conj(anp.cos(x)))
anp.cos.defgrad(   lambda ans, x : lambda g : - g * anp.conj(anp.sin(x)))
anp.tan.defgrad(   lambda ans, x : lambda g : g / anp.conj(anp.cos(x) **2))
anp.arcsin.defgrad(lambda ans, x : lambda g : g / anp.conj(anp.sqrt(1 - x**2)))
anp.arccos.defgrad(lambda ans, x : lambda g :-g / anp.conj(anp.sqrt(1 - x**2)))
anp.arctan.defgrad(lambda ans, x : lambda g : g / anp.conj((1 + x**2)))
anp.sinh.defgrad(  lambda ans, x : lambda g : g * anp.conj(anp.cosh(x)))
anp.cosh.defgrad(  lambda ans, x : lambda g : g * anp.conj(anp.sinh(x)))
anp.tanh.defgrad(  lambda ans, x : lambda g : g / anp.conj(anp.cosh(x) **2))
anp.arcsinh.defgrad(lambda ans, x : lambda g : g / anp.conj(anp.sqrt(x**2 + 1)))
anp.arccosh.defgrad(lambda ans, x : lambda g : g / anp.conj(anp.sqrt(x**2 - 1)))
anp.arctanh.defgrad(lambda ans, x : lambda g : g / anp.conj((1 - x**2)))
...
anp.square.defgrad( lambda ans, x : lambda g : g * 2 * anp.conj(x))
anp.sqrt.defgrad(   lambda ans, x : lambda g : g * 0.5 * anp.conj(x**-0.5))
anp.sinc.defgrad(   lambda ans, x : lambda g : g * anp.conj((anp.cos(anp.pi*x)*anp.pi*x - anp.sin(anp.pi*x))/(anp.pi*x**2)))
...
def make_grad_np_prod(ans, x, axis=None, keepdims=False): # TODO: Support tuples of axes.
    repeater, _ = repeat_to_match_shape(x, axis, keepdims)
    return lambda g: repeater(g * anp.conj(ans)) / anp.conj(x)
