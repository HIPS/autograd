...
anp.nan_to_num.defvjp(lambda vs, gvs, x: lambda g: anp.where(anp.isfinite(x), g, 0.))

# ----- Binary ufuncs -----

anp.add.defvjp(        lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g))
anp.add.defvjp(        lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g), argnum=1)
anp.multiply.defvjp(   lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, y * g))
anp.multiply.defvjp(   lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, x * g), argnum=1)
anp.subtract.defvjp(   lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g))
anp.subtract.defvjp(   lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, -g), argnum=1)
anp.divide.defvjp(     lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs,   g / y))
anp.divide.defvjp(     lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, - g * x / y**2), argnum=1)
anp.maximum.defvjp(    lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * balanced_eq(x, y)))
anp.maximum.defvjp(    lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * balanced_eq(y, x)), argnum=1)
anp.minimum.defvjp(    lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * balanced_eq(x, y)))
anp.minimum.defvjp(    lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * balanced_eq(y, x)), argnum=1)
anp.fmax.defvjp(       lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * balanced_eq(x, y)))
anp.fmax.defvjp(       lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * balanced_eq(y, x)), argnum=1)
anp.fmin.defvjp(       lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * balanced_eq(x, y)))
anp.fmin.defvjp(       lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * balanced_eq(y, x)), argnum=1)
anp.logaddexp.defvjp(  lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * anp.exp(x-ans)))
anp.logaddexp.defvjp(  lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * anp.exp(y-ans)), argnum=1)
anp.logaddexp2.defvjp( lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * 2**(x-ans)))
anp.logaddexp2.defvjp( lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g * 2**(y-ans)), argnum=1)
anp.true_divide.defvjp(lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g / y))
anp.true_divide.defvjp(lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, - g * x / y**2), argnum=1)
anp.mod.defvjp(        lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g))
anp.remainder.defvjp(  lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, g))
anp.mod.defvjp(        lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, -g * anp.floor(x/y)), argnum=1)
anp.remainder.defvjp(  lambda vs, gvs, ans, x, y : lambda g: unbroadcast(vs, gvs, -g * anp.floor(x/y)), argnum=1)
anp.power.defvjp(
    lambda vs, gvs, ans, x, y : lambda g:
...
anp.negative.defvjp(lambda vs, gvs, ans, x: lambda g: -g)
anp.abs.defvjp(
    lambda vs, gvs, ans, x : lambda g: g * replace_zero(anp.conj(x), 0.) / replace_zero(1.))
anp.fabs.defvjp(    lambda vs, gvs, ans, x : lambda g: anp.sign(x) * g)  # fabs doesn't take complex numbers.
anp.absolute.defvjp(lambda vs, gvs, ans, x : lambda g: g * anp.conj(x) / ans)
anp.reciprocal.defvjp(lambda vs, gvs, ans, x : lambda g: - g / x**2)
anp.exp.defvjp(   lambda vs, gvs, ans, x : lambda g: ans * g)
anp.exp2.defvjp(  lambda vs, gvs, ans, x : lambda g: ans * anp.log(2) * g)
anp.expm1.defvjp( lambda vs, gvs, ans, x : lambda g: (ans + 1) * g)
anp.log.defvjp(   lambda vs, gvs, ans, x : lambda g: g / x)
anp.log2.defvjp(  lambda vs, gvs, ans, x : lambda g: g / x / anp.log(2))
anp.log10.defvjp( lambda vs, gvs, ans, x : lambda g: g / x / anp.log(10))
anp.log1p.defvjp( lambda vs, gvs, ans, x : lambda g: g / (x + 1))
anp.sin.defvjp(   lambda vs, gvs, ans, x : lambda g: g * anp.cos(x))
anp.cos.defvjp(   lambda vs, gvs, ans, x : lambda g: - g * anp.sin(x))
anp.tan.defvjp(   lambda vs, gvs, ans, x : lambda g: g / anp.cos(x) **2)
anp.arcsin.defvjp(lambda vs, gvs, ans, x : lambda g: g / anp.sqrt(1 - x**2))
anp.arccos.defvjp(lambda vs, gvs, ans, x : lambda g:-g / anp.sqrt(1 - x**2))
anp.arctan.defvjp(lambda vs, gvs, ans, x : lambda g: g / (1 + x**2))
anp.sinh.defvjp(  lambda vs, gvs, ans, x : lambda g: g * anp.cosh(x))
anp.cosh.defvjp(  lambda vs, gvs, ans, x : lambda g: g * anp.sinh(x))
anp.tanh.defvjp(  lambda vs, gvs, ans, x : lambda g: g / anp.cosh(x) **2)
anp.arcsinh.defvjp(lambda vs, gvs, ans, x : lambda g: g / anp.sqrt(x**2 + 1))
anp.arccosh.defvjp(lambda vs, gvs, ans, x : lambda g: g / anp.sqrt(x**2 - 1))
anp.arctanh.defvjp(lambda vs, gvs, ans, x : lambda g: g / (1 - x**2))
anp.rad2deg.defvjp(lambda vs, gvs, ans, x : lambda g: g / anp.pi * 180.0)
anp.degrees.defvjp(lambda vs, gvs, ans, x : lambda g: g / anp.pi * 180.0)
anp.deg2rad.defvjp(lambda vs, gvs, ans, x : lambda g: g * anp.pi / 180.0)
anp.radians.defvjp(lambda vs, gvs, ans, x : lambda g: g * anp.pi / 180.0)
anp.square.defvjp( lambda vs, gvs, ans, x : lambda g: g * 2 * x)
anp.sqrt.defvjp(   lambda vs, gvs, ans, x : lambda g: g * 0.5 * x**-0.5)
anp.sinc.defvjp(   lambda vs, gvs, ans, x : lambda g: g * (anp.cos(anp.pi*x)*anp.pi*x - anp.sin(anp.pi*x))/(anp.pi*x**2))
anp.reshape.defvjp(lambda vs, gvs, ans, x, shape, order=None : lambda g: anp.reshape(g, vs.shape, order=order))
anp.roll.defvjp(   lambda vs, gvs, ans, x, shift, axis=None  : lambda g: anp.roll(g, -shift, axis=axis))
anp.array_split.defvjp(lambda vs, gvs, ans, ary, idxs, axis=0 : lambda g: anp.concatenate(g, axis=axis))
anp.split.defvjp(      lambda vs, gvs, ans, ary, idxs, axis=0 : lambda g: anp.concatenate(g, axis=axis))
anp.vsplit.defvjp(     lambda vs, gvs, ans, ary, idxs         : lambda g: anp.concatenate(g, axis=0))
anp.hsplit.defvjp(     lambda vs, gvs, ans, ary, idxs         : lambda g: anp.concatenate(g, axis=1))
anp.dsplit.defvjp(     lambda vs, gvs, ans, ary, idxs         : lambda g: anp.concatenate(g, axis=2))
anp.ravel.defvjp(  lambda vs, gvs, ans, x, order=None   : lambda g: anp.reshape(g, vs.shape, order=order))
anp.expand_dims.defvjp(lambda vs, gvs, ans, x, axis     : lambda g: anp.reshape(g, vs.shape))
anp.squeeze.defvjp(lambda vs, gvs, ans, x, axis=None    : lambda g: anp.reshape(g, vs.shape))
anp.diag.defvjp(   lambda vs, gvs, ans, x, k=0          : lambda g: anp.diag(g, k))
anp.flipud.defvjp( lambda vs, gvs, ans, x,              : lambda g: anp.flipud(g))
anp.fliplr.defvjp( lambda vs, gvs, ans, x,              : lambda g: anp.fliplr(g))
anp.rot90.defvjp(  lambda vs, gvs, ans, x, k=1          : lambda g: anp.rot90(g, -k))
anp.trace.defvjp(  lambda vs, gvs, ans, x, offset=0     : lambda g:
                    anp.einsum('ij,...->ij...', anp.eye(x.shape[0], x.shape[1], k=offset), g))
anp.full.defvjp(   lambda vs, gvs, ans, shape, fill_value, dtype=None : lambda g: anp.sum(g), argnum=1)
anp.triu.defvjp(   lambda vs, gvs, ans, x, k=0          : lambda g: anp.triu(g, k=k))
anp.tril.defvjp(   lambda vs, gvs, ans, x, k=0          : lambda g: anp.tril(g, k=k))
anp.clip.defvjp(   lambda vs, gvs, ans, x, a_min, a_max : lambda g: g * anp.logical_and(ans != a_min, ans != a_max))
anp.swapaxes.defvjp(lambda vs, gvs, ans, x, axis1, axis2: lambda g: anp.swapaxes(g, axis2, axis1))
anp.moveaxis.defvjp(lambda vs, gvs, ans, a, source, destination: lambda g:
                    anp.moveaxis(g, destination, source))
anp.rollaxis.defvjp(lambda vs, gvs, ans, a, axis, start=0: lambda g: anp.rollaxis(g, start - 1, axis) if start > axis
                                                 else anp.rollaxis(g, start, axis + 1))
anp.real_if_close.defvjp(lambda vs, gvs, ans, x : lambda g: match_complex(vs, g))
anp.real.defvjp(  lambda vs, gvs, ans, x   : lambda g: match_complex(vs, g))
anp.imag.defvjp(  lambda vs, gvs, ans, x   : lambda g: match_complex(vs, -1j * g))
anp.conj.defvjp(  lambda vs, gvs, ans, x   : lambda g: anp.conj(g))
anp.conjugate.defvjp(lambda vs, gvs, ans, x: lambda g: anp.conj(g))
anp.angle.defvjp( lambda vs, gvs, ans, x   : lambda g: match_complex(vs, g * anp.conj(x * 1j) / anp.abs(x)**2))
anp.where.defvjp( lambda vs, gvs, ans, c, x=None, y=None : lambda g: anp.where(c, g, anp.zeros(g.shape)), argnum=1)
anp.where.defvjp( lambda vs, gvs, ans, c, x=None, y=None : lambda g: anp.where(c, anp.zeros(g.shape), g), argnum=2)
anp.cross.defvjp(lambda vs, gvs, ans, a, b, axisa=-1, axisb=-1, axisc=-1, axis=None : lambda g:
                  anp.cross(b, g, axisb, axisc, axisa, axis), argnum=0)
anp.cross.defvjp(lambda vs, gvs, ans, a, b, axisa=-1, axisb=-1, axisc=-1, axis=None : lambda g:
                  anp.cross(g, a, axisc, axisa, axisb, axis), argnum=1)
anp.linspace.defvjp(lambda vs, gvs, ans, start, stop, num : lambda g: anp.dot(anp.linspace(1.0, 0.0, num), g))
anp.linspace.defvjp(lambda vs, gvs, ans, start, stop, num : lambda g: anp.dot(anp.linspace(0.0, 1.0, num), g), argnum=1)
...
def grad_np_sum(vs, gvs, x, ans, axis=None, keepdims=False, dtype=None):
