...
import cupy.fft as ffto
from .cupy_wrapper import wrap_namespace
from .cupy_vjps import match_complex
...
defvjp(fftshift,  lambda ans, x, axes=None : lambda g:
                 match_complex(x, acp.conj(ifftshift(acp.conj(g), axes))))
defvjp(ifftshift, lambda ans, x, axes=None : lambda g:
                 match_complex(x, acp.conj(fftshift(acp.conj(g), axes))))

@primitive
def truncate_pad(x, shape):
    # truncate/pad x to have the appropriate shape
    slices = [slice(n) for n in shape]
    pads = list(zip(acp.zeros(len(shape), dtype=int),
               acp.maximum(0, acp.array(shape) - acp.array(x.shape))))
    return acp.pad(x, pads, 'constant')[slices]
...
def make_rfft_factors(axes, resshape, facshape, normshape, norm):
    ...
    # For same reason could have used cupy rather than acp.
    # but we already imported acp, so use it instead.
    fac = acp.zeros(resshape)
    fac[...] = 2
