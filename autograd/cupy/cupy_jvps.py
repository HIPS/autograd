...
from .cupy_vjps import (untake, balanced_eq, match_complex, replace_zero,
...
defjvp_argnum(acp.array_from_args, lambda argnum, g, ans, args, kwargs: untake(g, argnum-2, vspace(ans)))
defjvp(acp._array_from_scalar_or_array, None, None,
...
# ----- Binary ufuncs -----
defjvp(acp.add,        lambda g, ans, x, y : broadcast(g, ans),
                       lambda g, ans, x, y : broadcast(g, ans))
defjvp(acp.subtract,   lambda g, ans, x, y : broadcast(g, ans),
                       lambda g, ans, x, y : broadcast(-g, ans))
defjvp(acp.divide,     'same',
                       lambda g, ans, x, y : - g * x / y**2)
defjvp(acp.maximum,    lambda g, ans, x, y : g * balanced_eq(x, ans, y),
                       lambda g, ans, x, y : g * balanced_eq(y, ans, x))
defjvp(acp.minimum,    lambda g, ans, x, y : g * balanced_eq(x, ans, y),
                       lambda g, ans, x, y : g * balanced_eq(y, ans, x))
defjvp(acp.fmax,       lambda g, ans, x, y : g * balanced_eq(x, ans, y),
                       lambda g, ans, x, y : g * balanced_eq(y, ans, x))
defjvp(acp.fmin,       lambda g, ans, x, y : g * balanced_eq(x, ans, y),
                       lambda g, ans, x, y : g * balanced_eq(y, ans, x))
defjvp(acp.logaddexp,  lambda g, ans, x, y : g * acp.exp(x-ans),
                       lambda g, ans, x, y : g * acp.exp(y-ans))
defjvp(acp.logaddexp2, lambda g, ans, x, y : g * 2**(x-ans),
                       lambda g, ans, x, y : g * 2**(y-ans))
defjvp(acp.true_divide,'same',
                       lambda g, ans, x, y : - g * x / y**2)
defjvp(acp.mod,        lambda g, ans, x, y : broadcast(g, ans),
                       lambda g, ans, x, y : -g * acp.floor(x/y))
defjvp(acp.remainder,  lambda g, ans, x, y : broadcast(g, ans),
                       lambda g, ans, x, y : -g * acp.floor(x/y))
defjvp(acp.power,      lambda g, ans, x, y : g * y * x ** acp.where(y, y - 1, 1.),
                       lambda g, ans, x, y : g * acp.log(replace_zero(x, 1.)) * x ** y)
defjvp(acp.arctan2,    lambda g, ans, x, y : g * y / (x**2 + y**2),
...
defjvp(acp.angle,  lambda g, ans, x   : match_complex(ans, g * acp.conj(x * 1j) / acp.abs(x)**2))
defjvp(acp.where,  None,
       lambda g, ans, c, x=None, y=None : acp.where(c, g, acp.zeros(acp.shape(g))),
...
def forward_grad_np_var(g, ans, x, axis=None, ddof=0, keepdims=False):
    ...
    return (2.0 * acp.sum(acp.real(g * x_minus_mean), axis=axis, keepdims=keepdims) /
            (num_reps - ddof))
...
def forward_grad_np_std(g, ans, x, axis=None, ddof=0, keepdims=False):
    ...
    x_minus_mean = acp.conj(x - acp.mean(x, axis=axis, keepdims=True))
    return (acp.sum(acp.real(g * x_minus_mean), axis=axis, keepdims=keepdims) /
            ((num_reps - ddof) * ans))
