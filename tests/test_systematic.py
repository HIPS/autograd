...
def test_ceil():    unary_ufunc_check(np.ceil, lims=[-1.5, 1.5], complex=False)
def test_cos():     unary_ufunc_check(np.cos)
def test_cosh():    unary_ufunc_check(np.cosh)
def test_deg2rad(): unary_ufunc_check(np.deg2rad, complex=False)
def test_degrees(): unary_ufunc_check(lambda x : np.degrees(x)/50.0, complex=False)
def test_exp():     unary_ufunc_check(np.exp)
def test_exp2():    unary_ufunc_check(np.exp2)
def test_expm1():   unary_ufunc_check(np.expm1)
def test_fabs():    unary_ufunc_check(np.fabs, complex=False)
def test_floor():   unary_ufunc_check(np.floor, lims=[-1.5, 1.5], complex=False)
...
def test_rad2deg(): unary_ufunc_check(lambda x : np.rad2deg(x)/50.0, complex=False)
def test_radians(): unary_ufunc_check(np.radians, complex=False)
...
def test_remainder(): binary_ufunc_check_no_same_args(np.remainder, complex=False)
def test_true_divide(): binary_ufunc_check(np.true_divide, lims_B=[0.3, 2.0], complex=False)
def test_mod(): binary_ufunc_check_no_same_args(np.mod,    lims_B=[0.3, 2.0], complex=False)
def test_true_divide_neg(): binary_ufunc_check(np.true_divide, lims_B=[-0.3, -2.0], complex=False)
def test_mod_neg(): binary_ufunc_check_no_same_args(np.mod,    lims_B=[-0.3, -2.0], complex=False)
