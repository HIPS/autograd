...
def test_matmul_broadcast(rng):
    R = rng.randn
    combo_check(np.matmul, [0, 1])([R(1, 2, 2)], [R(3, 2, 1)])


def test_tensordot_1(rng):
    R = rng.randn
    C = lambda *shape: rng.randn(*shape) + 1j * rng.randn(*shape)
...
def test_einsum_naked_sum(rng):
    R = rng.randn
    combo_check(np.einsum, [1, 2])(["k,nk->"], [R(5)], [R(10, 5)])


def test_einsum_naked_sum2(rng):
    R = rng.randn
    combo_check(np.einsum, [1])(["abcd->bd"], [R(3, 2, 3, 2)])


def test_einsum_naked_sum_ellipsis(rng):
    R = rng.randn
    combo_check(np.einsum, [1, 2])(["...k,...nk->..."], [R(3, 5)], [R(3, 10, 5)])


def test_einsum_no_output_indices(rng):
    R = rng.randn
...
def test_tril_3d(rng):
    R = rng.randn
    combo_check(np.tril, [0])([R(5, 5, 4)], k=[-1, 0, 1])


def test_triu_3d(rng):
    R = rng.randn
