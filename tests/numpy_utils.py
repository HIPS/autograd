...
def stat_check(fun, test_complex=True, **kwargs):
    ...
    if test_complex:
        c = rng.randn() + 0.1j * rng.randn()
        E = rng.randn(2, 3) + 0.1j * rng.randn(2, 3)
        check([x, c, A])
...
def unary_ufunc_check(fun, lims=[-2, 2], test_complex=True, **kwargs):
    ...
    vector = transform(lims, rng.rand(2))
    mat = transform(lims, rng.rand(3, 2))
    mat2 = transform(lims, rng.rand(1, 2))
    check = combo_check(fun, (0,), **kwargs)
    check([scalar, vector, mat, mat2])
    if test_complex:
        comp = transform(lims, 0.4) + 0.1j * transform(lims, 0.3)
        matc = transform(lims, rng.rand(3, 2)) + 0.1j * rng.rand(3, 2)
        check([comp, matc])
...
def binary_ufunc_check(fun, lims_A=[-2, 2], lims_B=[-2, 2], test_complex=True, **kwargs):
    ...
    if test_complex:
        comp = 0.6 + 0.3j
        matc = rng.rand(3, 2) + 0.1j * rng.rand(3, 2)
        check(
...
def binary_ufunc_check_no_same_args(fun, lims_A=[-2, 2], lims_B=[-2, 2], test_complex=True, **kwargs):
    ...
    if test_complex:
        comp1 = 0.6 + 0.3j
        comp2 = 0.1 + 0.2j
        matc1 = rng.rand(3, 2) + 0.1j * rng.rand(3, 2)
        matc2 = rng.rand(3, 2) + 0.1j * rng.rand(3, 2)
        check(
